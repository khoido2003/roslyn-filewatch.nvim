...
---@field notify fun(msg: string, level?: number)
---@field notify_roslyn_renames fun(client_id: number, files: roslyn_filewatch.RenameEntry[])
...
function M.on_create(client_id, path, st, snapshots, callbacks)
  ...
  if callbacks and callbacks.notify_roslyn_renames then
    pcall(callbacks.notify_roslyn_renames, client_id, { { old = del_ent.path, new_path = path } })
  end
